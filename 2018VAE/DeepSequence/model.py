...
class VariationalAutoencoder:
    ...
    def __init__(self,
       ...
       batch_size=100,
        ...
        print("self.decoder_architecture =",self.decoder_architecture)
    ...
    def decoder_sparse(self, x, layer_up_val):
        ...
        print("len(self.decoder_architecture) = ",len(self.decoder_architecture))
        ...
        if self.convolve_patterns:
            ...
            print("self.final_output_size = ",self.final_output_size)
            print("self.seq_len",self.seq_len)
            print("self.conv_decoder_size = ",self.conv_decoder_size)
            print("W_conv =",W_conv)
            print("W_out =",W_out)
            if self.sparsity != False:
    ...
    def create_gradientfunctions(self):
        ...
        print("all_likelihood_components = ",all_likelihood_components)
    ...
    def load_parameters(self, file_prefix=""):
        ...
        for name in p_list.keys():
           ...
           # print(self.params[name].set_value(p_list[name].astype(theano.config.floatX)))
            #print(self.m[name].set_value(m_list[name].astype(theano.config.floatX)))
            #print(self.v[name].set_value(v_list[name].astype(theano.config.floatX)))
...
class VariationalAutoencoderMLE:
    ...
    def decoder(self, x, layer_up_val):
        ...
        print(len(self.decoder_architecture))
        for layer_num in range(len(self.decoder_architecture)):
            w_name = 'W_decode_'+str(layer_num)
            b_name = 'b_decode_'+str(layer_num)
