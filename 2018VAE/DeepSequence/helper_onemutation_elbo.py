...
class DataHelper:
    ...
    def one_hot_3D_ver3(self, s):
        x_data = []
        for i in range(len(s)):
            x_data.append(np.zeros((len(s[i]),len(self.alphabet))))

        for i in range(len(s)):
            for j ,letter in enumerate(s[i]):
                x_data[i][j][self.aa_dict[letter]] = 1
        return x_data
    ...
    def delta_elbo(self, model, mutant_tuple_list, N_pred_iterations=10):
        ...
        print("self.wt_elbo =",self.wt_elbo)
        ...
        print("self.uniprot_focus_col_to_focus_idx = ",self.uniprot_focus_col_to_focus_idx)
        print("mutant_sequences = ",mutant_sequences)
    ...
    def single_mutant_matrix(self, model, N_pred_iterations=10, \
        ...
        for wt,pos,idx_focus in wt_pos_focus_idx_tuple_list:
            for mut in self.alphabet:
                if wt != mut:
                    ...
                    # Add t"o the list = ",               self.mutant_sequences.append("".join(focus_seq_copy))
        ...
        print("self.mutant_sequences_one_hot = ",self.mutant_sequences_one_hot)
        ...
        for i in range(N_pred_iterations):
            ...
            for j in range(0,self.mutant_sequences_one_hot.shape[0],minibatch_size):
                ...
                for k,idx_batch in enumerate(batch_index.tolist()):
                    ...
                    print("self.prediction_matrix =",self.prediction_matrix)
        # Then take the mean of all my elbo samples
        self.mean_elbos = np.mean(self.prediction_matrix, axis=1).flatten().tolist()
        print("self.mean_elbos =",self.mean_elbos)
        self.wt_elbo = self.mean_elbos.pop(0)
        print("self.wt_elbo = ",self.wt_elbo)
        self.mutant_sequences_descriptor.pop(0)
        print("self.mutant_sequences_descriptor =",self.mutant_sequences_descriptor)
        self.delta_elbos = np.asarray(self.mean_elbos) - self.wt_elbo
        print("np.asarray(self.mean_elbos) = ")
        print("self.delta_elbos = ",self.delta_elbos)
        if filename_prefix == "":
            print("self.mutant_sequences_descriptor =",self.mutant_sequences_descriptor)
            print("self.delta_elbos = ",self.delta_elbos)
    ...
    def custom_mutant_matrix_mutation_elbo(self, input_filename, model, N_pred_iterations=10, \
        ...
        for i in range(len(self.mutant_sequences)):
            print(self.mutant_sequences[i])
        for i,sequence in enumerate(self.mutant_sequences):
            for j,letter in enumerate(sequence):
                k = self.aa_dict[letter]
                self.mutant_sequences_one_hot[i,j,k] = 1.0
        ...
        self.delta_elbos = np.asarray(self.mean_elbos)
