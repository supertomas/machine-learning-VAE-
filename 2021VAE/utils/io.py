...
def load_gzdata_5remove(filename, one_hot=True):
    names, seqs = read_gzfasta_5remove(filename)
    if one_hot:
        seqs = to_one_hot(seqs)
    return names, seqs

def load_data(filename, one_hot=True):
    names, seqs = read_fasta(filename)
    if one_hot:
        seqs = to_one_hot(seqs)
    return names, seqs
...
def read_gzfasta_5remove(filepath, output_arr=False, encoding='utf-8'):
    names = []
    seqs = []
    seq = ''
    with gzip.open(filepath, 'rb') as fin:
            ...
            if line:
                if line[0] == '>':
                    if seq:
                        names.append(name)
                        if output_arr:
                            seqs.append(np.array(list(seq[:-5])))
                        else:
                            seqs.append(seq[:-5])
                    name = line[1:]
                    seq = ''
